...
export function ThemeSettingsPage({ projectId }: ThemeSettingsPageProps) {
  ...
  const loadThemes = async () => {
    try {
      setLoading(true)
      const result = await api.theme.list(projectId)
  ...
  const handleApply = async () => {
    ...
    try {
      await api.theme.create(projectId, currentPreset)
  ...
  const handleActivate = async (themeId: string) => {
    try {
      await api.theme.activate(projectId, themeId)
  ...
  const handleDelete = async (themeId: string) => {
    try {
      await api.theme.delete(projectId, themeId)
  ...
  return (
    <div data-testid="theme-settings-page" className="p-8 space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Upload and manage themes for this project</p>
      </div>
