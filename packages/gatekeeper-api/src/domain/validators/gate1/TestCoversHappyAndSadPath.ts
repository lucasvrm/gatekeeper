...
export const TestCoversHappyAndSadPathValidator: ValidatorDefinition = {
  ...
  async execute(ctx: ValidationContext): Promise<ValidatorOutput> {
    ...
    try {
      ...
      const happyKeywordsStr = ctx.config.get('HAPPY_PATH_KEYWORDS') || 'success,succeeds,should,valid,passes,correctly,works,returns,deve,válido,sucesso,corretamente,retorna,funciona'
      const sadKeywordsStr = ctx.config.get('SAD_PATH_KEYWORDS') || 'error,fail,throws,invalid,not,reject,deny,block,erro,falha,inválido,não deve,rejeita,bloqueia'
